...
(function () {
  ...
  function shouldShowGuide() {
    if (window.APJ_ENABLE_GUIDE === false) return false;
    if (activePath() === 'dashboard.html') return false;
    return true;
  }
  ...
  function icon(name) {
    ...
    const map = {
      ...
      help:'M9.1 9a3 3 0 1 1 5.8 1c0 2-2.9 2.2-2.9 4.2M12 18h.01M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0',
      logout:'M10 17l5-5-5-5M15 12H3m7-9h7a2 2 0 0 1 2 2v3m0 8v3a2 2 0 0 1-2 2h-7',
  ...
  function linkHtml(item) {
    const locked = window.APJAuth && !window.APJAuth.hasPermission(item.permission);
    const active = item.url && isActive(item.url);
    const href = item.url || '#';
    return '<a href="' + href + '" class="apj-nav-link ' + (active ? 'apj-active ' : '') + (locked ? 'apj-locked' : '') + '" data-url="' + href + '" data-permission="' + (item.permission || '') + '" data-locked="' + (locked ? 'Y' : 'N') + '"><span class="apj-nav-icon">' + icon(item.icon) + '</span><span class="apj-nav-text">' + item.name + '</span>' + (locked ? '<span class="apj-lock-badge">🔒 Lock</span>' : '') + '</a>';
  }
  ...
  function build() {
    ...
    const guideButton = shouldShowGuide() ? '<button class="apj-btn-guide" type="button" data-apj-guide aria-label="Buka panduan"><span class="apj-footer-icon">' + icon('help') + '</span><span class="apj-footer-text">Panduan</span></button>' : '';
    html += '</nav><div class="apj-sidebar-footer">' + guideButton + '<button class="apj-btn-logout" type="button" data-apj-logout aria-label="Keluar sistem"><span class="apj-footer-icon">' + icon('logout') + '</span><span class="apj-footer-text">Keluar</span></button></div></aside>';
